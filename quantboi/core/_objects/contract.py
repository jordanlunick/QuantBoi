...
def create_option_symbol_string(
        ...
        option_type: OptType, strike_price: float) -> str:
    """
    ...
    option_type_str = 'C' if option_type == OptType.Call else 'P'
...
class BaseContract:
    ...
    security_type: Optional[SecType] = field(default=None) # Security type (Stock, Option, Future, etc.)
    ...
    # /// Option Attributes ///
    option_type: Optional[OptType] = field(default=None) # Option type (Call=1 or Put=-1)
...
class OptionContract(BaseContract):
    def __init__(
        ...
        BaseContract.__init__(
            self, security_type=SecType.Option, symbol=stock.symbol, option_type=option_type,
